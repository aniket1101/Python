...
def load_files(directory):
    ...
    """
    files_mapping ={}
    for name_of_file in os.listdir(directory):
        with open(os.path.join(directory, name_of_file)) as f:
            files_mapping[name_of_file] = f.read()
...
def compute_idfs(documents):
    ...
    for word in words:
        ...
        idf = math.log(total_documents / f)
        idfs[word] = idf
    return idfs
    ...
    for sentence in sentences:
        ...
        for word in query:
            if word in sentences[sentence]:
                words_found += 1
                idf += idfs[word]
        density = float(words_found) / len(sentences[sentence])
...
# When was the first work of Artificial Intelligence published?
# In what case are A and B called mutually exclusive events?
